...
- 反馈：松树锥形不明显（树干短、锥心被树干戳破）；棕榈冠干分离、叶子要下垂
- 松树：圆锥覆盖整个冠区（含中心，树干被锥体包裹，锥底只露一小段棕干），底座低、树冠高（18 层）
- 棕榈：高细棕干（trunkLayers 24 / trunkR 0.8 / 中心强制树干，不依赖 QR 中心是否深色）+ 顶部 2 层大叶（s=2 放大）微下垂
- 方块缩放支持：canopyLayers 返回 {y, s}，applyToMesh 按 s 缩放（棕榈大片叶）
- 测试：松树锥覆盖中心 / 棕榈下垂放大叶；75/75 全绿
- vision 验证：松树高尖绿锥、棕榈高干+顶冠，均与圆顶/平顶/高柱区分明显

## 树种精简与松树树干修正（2026-08-27）

- 反馈：松树树干太短；枫树/银杏/棕榈效果不佳 → 只保留樱花/松树；下拉框样式统一；自动旋转移到 3D 视图右下角；中间只留下拉+扁平按钮
- TREE_TYPES 精简为 sakura/pine；删除 maple/ginkgo/palm 及棕榈专属逻辑（方块缩放 s、强制干等）
- 松树：圆锥覆盖整个冠区 + 中心强制 12 层树干（不依赖 QR 中心深浅），锥底中心高（露树干）、边缘低（近地面）
- UI：select 样式对齐按钮；#spin 移入 .stage 右下角；.controls 只留 select + 扁平按钮
- 测试精简至 73 个；vision 验证「棕树干 + 绿锥冠」松树形态

## 松树树干加高（2026-08-27 第三轮）

- 反馈：树干仍未加高。根因：树干渲染了但被周围宽锥体遮挡（相机视角下绿色盖住棕干）
- 修复：中心强制 3×3 实心高树干（24 层）+ 锥体抬离地面（锥底中心 20/边缘 12）+ 冠幅收窄 0.7
- 像素实测：可见棕色 245px / 树总高 409px = 60%（此前 ~25%）；vision 确认粗壮棕干 + 绿色锥冠

## 扁平视图配色跟随树种 + 按钮文字切换（2026-08-27）

- 反馈：松树 3D 无红色但扁平视图是红的（写死深玫红）；按钮应随视图切换文字
- flat.js：matrixToColoredRgba 接收树种，扁平配色 = 该树 3D 配色加深到可扫亮度（toScannable，目标亮度 80）
- UI：drawFlat2d 传 tree；扁平/3D 切换按钮文字互切（扁平扫码视图 ↔ 3D 视图），?flat 深链同步
- 测试：+2（松树扁平树冠为深绿非红、松树配色仍可解码）；75/75
- vision 验证：松树扁平视图深绿主体 + 按钮显示「3D 视图」
